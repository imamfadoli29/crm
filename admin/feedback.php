    ...
    <div class="card">
        ...
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalLong"><i class="fa fa-plus-circle"></i> Tambah Feedback</button><br/><br/>
            ...
            <table id="dataTable" class="table table-striped">
                ...
                <tbody>
                    <?php foreach($getAllFeedback as $data){ $row = $db->object($data); ?>
                    <tr>
                    <td><?= $row->id_customer ?></td>
                    <td><?= $row->isi_feedback ?></td>
                    <td><?= $row->feedback_date ?></td>
                    <td>
                        <a href="../controller/Data_crm.php?action=hapus_feedback&id=<?= $row->id_feedback; ?>" class="h5"><i class="fa fa-trash text-danger"></i></a>
